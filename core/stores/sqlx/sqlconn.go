...
type (
	...
	connProvider func() (*sqlx.DB, error)
...
func NewSqlConnFromDB(db *sqlx.DB, opts ...SqlOption) SqlConn {
	conn := &commonSqlConn{
		connProv: func() (*sqlx.DB, error) {
			return db, nil
		},
...
func (db *commonSqlConn) RawDB() (*sqlx.DB, error) {
	return db.connProv()
}
